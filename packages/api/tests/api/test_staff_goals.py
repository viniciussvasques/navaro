...
async def test_staff_goals_workflow(client: AsyncClient, auth_headers: dict):
    ...
    resp = await client.post(
        f"/api/v1/establishments/{est_id}/services", json=service_payload, headers=headers
    )
    assert resp.status_code == 201
    ...
    await client.post(
        f"/api/v1/services/{service_id}/staff", json={"staff_ids": [staff_id]}, headers=headers
    )
    ...
    for i in range(4):
        ...
        await client.patch(
            f"/api/v1/appointments/{new_appt_id}", json={"status": "completed"}, headers=headers
        )
