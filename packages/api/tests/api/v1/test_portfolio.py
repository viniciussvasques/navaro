...
async def test_portfolio_flow(
    ...
    resp = await client.post("/api/v1/portfolio", headers=auth_headers, json=image_data)
    ...
    resp = await client.post("/api/v1/portfolio", headers=auth_headers, json=staff_image_data)
    assert resp.status_code == 201
    ...
    # ----------------------------------------------------------------------------------
    resp = await client.delete(f"/api/v1/portfolio/{image_id}", headers=auth_headers_second_user)
    assert resp.status_code == 403

    # 6. Delete Image
    # ----------------------------------------------------------------------------------
    resp = await client.delete(f"/api/v1/portfolio/{image_id}", headers=auth_headers)
