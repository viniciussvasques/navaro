...
async def test_geo_search_radius(client: AsyncClient, auth_headers: dict):
    ...
    resp = await client.patch(
        f"/api/v1/establishments/{est_near_id}",
        json={
    ...
    resp = await client.patch(
        f"/api/v1/establishments/{est_far_id}",
        json={
    ...
    resp = await client.get(
        f"/api/v1/establishments?lat={sp_center['lat']}&lng={sp_center['lng']}&radius=2"
    )
    assert resp.status_code == 200
    data = resp.json()
    items = data["items"]
    ...
    for item in items:
        print(
            f"DEBUG: - {item['name']} ({item['id']}) Distance: {item.get('distance')} km Status: {item['status']}"
        )
    ...
    # --------------------------------------------------------------------------
    resp = await client.get(
        f"/api/v1/establishments?lat={sp_center['lat']}&lng={sp_center['lng']}&radius=15"
    )
    assert resp.status_code == 200
    data = resp.json()
    items = data["items"]
    ...
    assert idx_near < idx_far, (
        f"Near ID should be before Far ID. Near index: {idx_near}, Far index: {idx_far}"
    )
