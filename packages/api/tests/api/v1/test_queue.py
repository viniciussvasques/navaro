...
async def test_queue_flow(
    ...
    # ----------------------------------------------------------------------------------
    resp = await client.delete(f"/api/v1/queue/{entry1_id}", headers=auth_headers)
    ...
    resp = await client.patch(
        f"/api/v1/queue/{entry2_id}/status", headers=auth_headers, json={"status": "called"}
    )
    assert resp.status_code == 200
    ...
    # ----------------------------------------------------------------------------------
    resp = await client.patch(
        f"/api/v1/queue/{entry2_id}/status", headers=auth_headers, json={"status": "serving"}
    )
    assert resp.status_code == 200
    ...
    # ----------------------------------------------------------------------------------
    resp = await client.patch(
        f"/api/v1/queue/{entry2_id}/status", headers=auth_headers, json={"status": "completed"}
    )
    assert resp.status_code == 200
    ...
    resp = await client.get(
        f"/api/v1/queue/establishments/{establishment_id}?status_filter=waiting"
    )
    data = resp.json()
