...
async def promote_to_admin(email_or_phone: str, db):
    await db.execute(
        update(User).where(User.phone == email_or_phone).values(role=UserRole.admin)
    )
    await db.commit()
...
async def test_wallet_payment_flow(client: AsyncClient, auth_headers: dict, app):
    ...
    svc_resp = await client.post(f"/api/v1/establishments/{est_id}/services", json={
        "name": "Corte Wallet", "price": 50.0, "duration_minutes": 30
    }, headers=headers)
    svc_id = svc_resp.json()["id"]
    
    staff_resp = await client.post(f"/api/v1/establishments/{est_id}/staff", json={
        "name": "Barber Wallet", "role": "barber", "commission_rate": 20.0
    }, headers=headers)
...
async def test_cashback_commission_and_referral(client: AsyncClient, auth_headers: dict, app):
    ...
    async for db in app.dependency_overrides[get_db]():
        await db.execute(update(User).where(User.phone == "+5511988888888").values(referred_by_id=user_b_id))
    ...
    await client.put(f"/api/v1/admin/settings/{SettingsKeys.CASHBACK_ENABLED}", json={"value": "true"}, headers=headers)
    await client.put(f"/api/v1/admin/settings/{SettingsKeys.CASHBACK_PERCENT}", json={"value": "10.0"}, headers=headers)
    await client.put("/api/v1/admin/settings/referral_bonus_amount", json={"value": "7.0"}, headers=headers)

    # 2. Setup Establishment
    est_payload = {"name": "Full Flow Salon", "category": "barbershop", "address": "Rua Full, 1", "city": "SP", "state": "SP", "phone": "11900000000", "business_hours": {d: {"open": "00:00", "close": "23:59"} for d in ["mon", "tue", "wed", "thu", "fri", "sat", "sun"]}}
    est_resp = await client.post("/api/v1/establishments", json=est_payload, headers=headers)
    est_id = est_resp.json()["id"]
    svc_resp = await client.post(f"/api/v1/establishments/{est_id}/services", json={"name": "Corte Full", "price": 100.0, "duration_minutes": 30}, headers=headers)
    svc_id = svc_resp.json()["id"]
    ...
    login_s = await client.post("/api/v1/auth/verify", json={"phone": staff_user_phone, "code": code_s})
    staff_uid = login_s.json()["user"]["id"]
    
    staff_resp = await client.post(f"/api/v1/establishments/{est_id}/staff", json={
        "name": "Staff Worker", "role": "barber", "commission_rate": 20.0, "user_id": staff_uid
    }, headers=headers)
    staff_id = staff_resp.json()["id"]

    # 3. Complete Appointment
    appt_payload = {"establishment_id": est_id, "service_id": svc_id, "staff_id": staff_id, "scheduled_at": (datetime.now(UTC) + timedelta(hours=1)).isoformat(), "payment_method": "cash", "payment_type": "single"}
    appt_resp = await client.post("/api/v1/appointments", json=appt_payload, headers=headers)
    appt_id = appt_resp.json()["id"]
    
    await client.patch(f"/api/v1/appointments/{appt_id}", json={"status": "completed"}, headers=headers)
