...
async def main():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        if not code:
            log("❌ Could not extract code from response. Is environment 'development'?", Colors.FAIL)
        ...
        if resp.status_code == 200:
            user = resp.json()
            log(f"✅ User Info: {user['name']} ({user['phone']}) - Role: {user['role']}", Colors.OKGREEN)
        else:
        ...
        if resp.status_code == 201:
            ...
            log(f"✅ Establishment Created: {est['name']} (ID: {est_id})", Colors.OKGREEN)
             ...
             log(f"⚠️ Establishment slug already exists, skipping create.", Colors.WARNING)
        ...
        resp = await client.post(f"{BASE_URL}/establishments/{est_id}/services", json=service_data, headers=headers)
        if resp.status_code == 201:
            svc = resp.json()
