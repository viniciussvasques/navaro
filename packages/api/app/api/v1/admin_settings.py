...
async def list_settings(
    db: DBSession, admin: AdminUser, category: str | None = None
...
async def get_setting(key: str, db: DBSession, admin: AdminUser) -> SettingResponse:
...
async def update_setting(
    key: str, request: SettingUpdate, db: DBSession, admin: AdminUser
) -> SettingResponse:
    """Update a setting value (admin only)."""
    service = SettingsService(db)
    setting = await service.set(key=key, value=request.value, description=request.description)
...
async def create_setting(
    request: SettingCreate, db: DBSession, admin: AdminUser
) -> SettingResponse:
...
async def delete_setting(key: str, db: DBSession, admin: AdminUser) -> None:
...
async def seed_default_settings(db: DBSession, admin: AdminUser) -> dict:
