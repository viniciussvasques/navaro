...
async def get_establishment_profile(
    ...
    services_result = await db.execute(
        select(Service)
        .where(Service.establishment_id == establishment_id, Service.active == True)
    ...
    staff_result = await db.execute(
        select(StaffMember)
        .where(StaffMember.establishment_id == establishment_id, StaffMember.active == True)
    ...
    gallery_result = await db.execute(
        ...
        .order_by(desc(PortfolioImage.created_at))
        .limit(20)
    )
